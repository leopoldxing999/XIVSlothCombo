...
public class Translatezh_CN
{
    ...
    public static Dictionary<string, string> db = new()
    {
        ...
        {"Applies Soteria when the selected target is at or above the set HP percentage.","当所选目标HP百分比在设定值以下时，对其使用拯救。"},
        {"Custom Zoe Feature","自定义活化设置"},
        {"Applies Zoe when the selected target is at or above the set HP percentage.","当所选目标HP百分比在设定值以下时，对其使用活化。"},
        {"Custom Pepsis Feature","自定义消化设置"},
        {"Triggers Pepsis if a shield is present and the selected target is at or above the set HP percentage.","当所选目标存在盾值且HP百分比在设定值以下时，激活消化。"},
        {"Custom Taurochole Feature","自定义白牛清汁设置"},
        {"Adds Taurochole when the selected target is at or above the set HP percentage.","当所选目标HP百分比在设定值以下时，插入白牛清汁。"},
        {"Custom Haima Feature","自定义输血设置"},
        {"Adds Haima when the selected target is at or above the set HP percentage.","当所选目标HP百分比在设定值以下时，插入输血。"},
        ...
        {"Applies Krasis when the selected target is at or above the set HP percentage.","当所选目标HP百分比在设定值以下时，对其使用混合。"},
        {"Physis Feature","自生设置"},
        {"Adds Physis.","自动插入自生。"},
        ...
        {"Adds Druochole when the selected target is at or above the set HP percentage.","当所选目标HP百分比在设定值以下时，自动插入灵橡清汁。"},
        #endregion
        #region SAMURAI
        {"Yukikaze Combo","雪风连"},
        ...
        {"Replace Gekko with its combo chain.\nIf all sub options are selected will turn into a full one button rotation (Advanced Samurai)","用月光连击的组合代替月光。\n如果所有子选项都被选中，将变成一个完整的一键单体循环（高级武士）。"},
        ...
        {"Adds Kasha Combo to Main Combo. Will add Kasha during Meikyo Shisui as well.","将花车连击加入主循环。在明镜止水期间也将使用花车连击。"},
        ...
        {"Oka to Mangetsu Combo","将樱花连击加入AOE连击"},
        ...
        {"Adds Tenka Goken and Midare: Setsugekka and their relevant Kaeshi when ready and when you're not moving to Mangetsu combo.","在未移动和技能冷却完毕的情况下，将纷乱雪月花、天下五剑和对应的回返技能加入循环。"},
        ...
        #endregion
        #region Other
        {"Adds True North on all ST Combos if Meikyo Shisui's buff is on you.","如果拥有明镜止水的BUFF，则所有单体循环组合都会使用真北。"},
        #endregion
        #endregion
        #region SCHOLAR
        #region DPS
        ...
        {"Delirium on Cooldown","血乱插入使用 设置"},
        {"Adds Delirium to main combo on cooldown and when Darkside is up. Will also spend 50 blood gauge if Delirium is nearly ready to protect from overcap.","当自身存在暗影状态且血乱结束冷却时插入血乱.同样当血乱即将结束冷却时会根据自身暗血值先插入使用血溅/寂灭以防止暗血值溢出."},
        {"Blood Weapon on Cooldown","嗜血插入使用 设置"},
        ...
        {"Use Tetragrammaton on oGCD Option","插入使用神名 设置."},
        {"Use Tetragrammaton on GCD Option","优先使用神名 设置"},
        {"Use Divine Benison on oGCD Option","插入使用神祝祷 设置"},
