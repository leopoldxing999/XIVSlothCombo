...
public class Translatezh_CN
{
    ...
    public static Dictionary<string, string> db = new()
    {
        ...
        {"Global Interrupt Feature","打断功能（全局）"},
        {"Replaces Stun (LowBlow) with interrupt (Interject) when the target can be interrupted.","当目标能被打断时，将下踢替换为插言。"},
        {"Global Raise Feature","复活功能（全局）"},
        {"Replaces Swiftcast with Raise/Resurrection/Verraise/Ascend/Egeiro when appropriate.","在合适的时机，将即刻咏唱替换为复活。"},
        #endregion
        #region ASTROLOGIAN
        ...
        {"Spread Shot turns into Scattergun when lvl 82 or higher, Both turn into Auto Crossbow when overheated\nand Bioblaster is used first whenever it is off cooldown.","等级82级以上，散射变为霰弹枪，当过热时，两者都会变成自动弩；毒菌冲击CD结束时，优先使用毒菌冲击。"},
        {"Drill","钻头"},
        ...
        {"Adds Mug to the combo.","将 夺取 加入到连击序列中."},
        ...
        {"Adds Bunshin and Phantom Kamaitachi to the combo.","将 分身之术 和 残影镰鼬 加入连击序列."},
        ...
        {"Replace Single target and AoE combo with Fell Cleave/Decimate during Inner Release","在原初的解放状态下，将暴风斩连击替换为裂石飞环/地毁人亡"},
        ...
        {"Adds Inner Release to Storm's Path Combo when Nascent Chaos is not up.","在没有原初的混沌BUFF时，将原初的解放加入到暴风斩连击"},
        {"Infuriate on Storm's Path","战壕加入暴风斩循环"},
        {"Adds Infuriate to Storm's Path Combo when gauge is below 50 and not under Inner Release.","在没有原初的解放BUFF且不会溢出兽魂时，将战壕加入到暴风斩连击"},
        #endregion
        #region WHITE MAGE
        ...
        {"Adds Thin Air to the WHM Raise Feature/Alternative Feature","为白魔复活特性/替代添加插入无中生有"},
        #endregion
        #region DOH
        ...
        "出卡后，切换回你之前手动选择的目标。(也可以是你出牌的对象）。"},
        {"Target Focus Feature","选择焦点目标功能"},
        {"Once you've played your card, switch back to your focus target.","出卡后，切换回你的焦点目标。"},
        #endregion
        #endregion
        #region BLACK MAGE
        // Fire     1/2/3/4     火炎 烈炎 爆炎 炽炎
        // Blizzard 1/2/3/4     冰结 冰冻 冰封 冰澈
        // Thunder  1/2/3/4     闪雷 震雷 暴雷 霹雷
        {"Scathe Feature","崩溃功能"},
        {"Replaces Scathe with Fire 4 or Blizzard 4 depending on Astral Fire/Umbral Ice.","根据星极火/灵极冰的情况，用炽炎或冰澈替换崩溃。"},
        ...
        {"Replaces Scathe with Thunder 1/3 when the debuff isn't present or expiring and Thundercloud is available.","当目标的雷Debuff不存在或即将失效，且雷云已触发时，用闪雷/暴雷替换崩溃。"},
        {"Despair Option","绝望功能"},
        {"Replaces Fire 4 with Despair when below 2400 MP.","2400MP以下时，用炽炎替换绝望。"},
        {"Simple AoE Feature","简易 AoE 功能"},
        {"Replaces Flare with a full one button rotation.","将核爆整合为一键AoE循环。"},
        {"Aspect Swap Option","等待翻译"},
        {"Replaces Scathe with Blizzard 3 when at 0 MP in Astral Fire or with Fire 3 when at 10000 MP in Umbral Ice with 3 Umbral Hearts.","在星极火中空蓝时，用冰封替换崩溃；在有3档灵极心的灵极冰中满蓝时，用爆炎替换崩溃。"},
        {"Thunder 1/3 Option","闪雷/暴雷功能"},
        {"Replaces Scathe with Thunder 1/3 when the debuff isn't present or expiring.","当目标的雷Debuff不存在或即将失效，用闪雷/暴雷替换绝望。"},
        ...
        {"Adds Burst Strike and Hypervelocity (when available) to Main Combo when under No Mercy and Gnashing Fang is over.","当在无情状态下且已使用过烈牙后，在主连击中插入爆发击和超高速(若可用)."},
        ...
        {"Main Combo on Demolish Option","一键循环集中于破碎拳"},
        {"Replaces Demolish with the Main Combo, except without any oCDs added. Useful for saving burst.","一键循环集中于破碎拳，但不使用任何能力技。用来攒爆发。"},
        ...
        "在神秘环持续期间会在50魂衣时使用夜游魂衣(模拟2分钟双夜游魂衣爆发期)，但是会在爆发期外囤积魂衣。\n低于88级时，会在50魂衣时使用夜游魂衣。"},
        ...
        {"Replace Spinning Scythe with its combo chain.\nIf all sub options are toggled will turn into a full one button rotation (Simple AOE)","将旋转钐割替换为旋转钐割连击。\n如果启用了所有子选项将变为一键循环(简易AOE)"},
        {"Adds Soul Scythe to AoE Combo when Soul Gauge is 50 or less and current target is afflicted with Death's Design.","当魂衣值低于50点，并且目标拥有死亡烙印Debuff时，将灵魂切割加入连击。" },
        {"Adds Whorl of Death to AoE Combo if Death's Design is not present on current target, or is about to expire.","当目标没有或者死亡烙印Debuff即将到期时，将死亡之涡加入连击。"},
        ...
        {"Adds Enshroud Combo (Void/Cross Reaping, Communio, and Lemure's Slice) on Blood Stalk and Grim Swathe.","将夜游魂衣连击(虚无收割/交错收割,团契,夜游魂切割/钐割)加入到隐匿挥割/束缚挥割"},
        #endregion
        ...
        {"Adds Soulsow to Slice, Spinning Scythe, Shadow of Death, Harpe, and Blood Stalk when out of combat.","在非战斗状态下将切割/旋转钐割/死亡之影/勾刃/隐匿挥割替换为播魂种。"},
        ...
        {"Stack Reposte Combo on specified action(s)\n**Must be in melee range or have Gap close with Corps-a-corps enabled**","整合魔六连至下列选项之一\n**必须处于近战攻击距离内或可用短兵相接接近目标**"},
        ...
        {"Adds Lucid Dreaming to Dosis when MP drops below slider value","当MP到达设定值时插入醒梦."},
        {"Eukrasian Dosis Option","均衡注药 设置"},
        {"Automatic DoT Uptime","自动插入均衡注药续Dot."},
        {"Toxikon Movement Option","移动时使用箭毒 设置"},
        {"Use Toxikon when you have Addersting charges and are moving","当你拥有一个以上的蛇刺则在移动时插入箭毒."},
        #endregion
        #region AoE DPS Feature
        {"No Phlegma to Toxikon Option","箭毒替换 选项1"},
        {"Use Toxikon when out of Phlegma charges\nTakes priority over Dyskrasia","当发炎全部使用完后将替换为箭毒.\n若没有蛇刺,且开启了失衡替换相关选项,则替换为失衡."},
        {"Toxikon Distance Option","箭毒替换 选项2"},
        {"Use Toxikon when out of Phlemga's Range\nTakes priority over Dyskrasia","当选择目标不在发炎的攻击距离内时替换为箭毒.\n若没有蛇刺,且开启了失衡替换相关选项,则替换为失衡."},
        {"No Phlegma to Dyskrasia Option","发炎替换 选项1"},
        {"Use Dyskrasia when out of Phlegma charges","当选择目标不在发炎的攻击距离内时替换为发炎."},
        {"Dyskrasia No-Target Option","发炎替换 选项2"},
        {"Use Dyskrasia when no target is selected","当未选择攻击目标时为发炎."},
        {"Adds Lucid Dreaming to Phlegma when MP drops below slider value","当MP到达设定值时插入醒梦."},
        #endregion
        #region Diagnosis Simple Single Target Heal
        {"Diagnosis Simple Single Target Heal Feature","整合一键治疗至诊断"},
        ...
        {"Enables features tied to Ruin, or Ruin II.\nIf all sub options are toggled will turn into a full one button rotation (Simple Summoner)\nRuin III is kept untouched for mobility.","单体连击集中于毁灭/毁坏。\n如果所有的子选项都勾选，会变成一键单体循环。\n毁荡留给走位，不会自动使用。"},
        {"Enable AoE Combo Features","启用 AoE 连击功能"},
        {"Enables features tied to Tridisaster.\nIf all sub options are toggled will turn into a full one button rotation (Simple AOE)","AoE 连击集中于三重灾祸。\n如果所有的子选项都勾选，会变成一键群体循环。"},
        {"Demi Attacks on Main Combo","亚灵神攻击进循环"},
        {"Adds Deathflare/Ahk Morn/Revelation to the Main Combo.","添加死星核爆/死亡轮回/天启到单体循环。"},
        {"AoE Demi Attacks on AoE Combo","亚灵神攻击进 AoE 循环"},
        {"Adds Deathflare/Ahk Morn/Revelation to the AOE Combo.","添加死星核爆/死亡轮回/天启到 AoE 循环。"},
        {"Gemshine on Main Combo","宝石耀进循环"},
        {"Adds Egi Attacks (Gemshine) to the Main Combo.","蛮神之灵攻击进循环。"},
        {"Adds Slipstream on RuinI/Ruin II/Tri-disaster.","螺旋气流替换毁灭/毁坏/三重灾祸。"},
        {"Adds Crimson Cyclone/Crimson Strike on RuinI/Ruin II/Tri-disaster.","深红旋风、深红强袭替换毁灭/毁坏/三重灾祸。"},
        {"Adds Mountain Buster on RuinI/Ruin II/Tri-disaster.","山崩替换毁灭/毁坏/三重灾祸。"},
        {"Change Fester into Energy Drain when out of Aetherflow stacks.","以太超流档数用光后，将溃烂爆发替换为能量吸收。"},
        {"Reminds you to summon Carbuncle by replacing most actions with Summon Carbuncle.","提醒你召唤宝石兽，把大多数技能替换成宝石兽召唤。"},
        {"Ruin 4 on Main Combo","毁绝进循环"},
        {"Adds Ruin 4 to the Main Combo when there are currently no summons active.","当前没有召唤物时打毁绝。"},
        {"Adds Ruin4 on AoE Combo when there are currently no summons active.","当前没有召唤物时，AoE 打毁绝。"},
        {"Ruin 4 Fester Option","毁绝 溃烂爆发 选项"},
        {"Changes Fester to Ruin 4 when out of Aetherflow stacks, Energy Drain is on cooldown, and Ruin 4 is up.","当以太超流档数用光，能量吸收在冷却，毁绝能用时，用毁绝替换溃烂爆发。"},
        {"Energy Drain/Fester on Main Combo","能量吸收/溃烂爆发进循环"},
        {"Adds ED/Fester to the Main Combo. Will use on cooldown.","能量吸收/溃烂爆发能用的时候会进循环。"},
        {"Egi Summon Order","蛮神之灵召唤顺序"},
        {"Sets the order you summon egis.","设置你召唤蛮神之灵的顺序。"},
        {"Energy Siphon/Painflare on AoE Combo","能量吸收/痛苦核爆进 AoE 循环"},
        {"Adds Energy Siphon/Painflare to the AoE Combo","将能量吸收/痛苦核爆加入 AoE 循环"},
        {"Searing Light on Single Target/AoE combo","灼热之光进单体/AoE 循环"},
        {"Adds Searing Light to the Single target, and AoE combos. Will be used on cooldown.","灼热之光冷却结束时，将其加入单体和 AoE 循环。"},
        {"Searing Light Burst Option","灼热之光爆发设置"},
        {"Casts Searing Light only during Bahamut/Phoenix Phase.\nChoose which phase to burst in under 'Burst Phase Choice' option.\nNot recommended for SpS Builds.","只在龙神/不死鸟阶段使用灼热之光。\n“爆发阶段选择”选项中可以具体设置。\n高咏速装备不建议使用。"},
        {"Demi Summons on Main Combo","亚灵神召唤进循环"},
        {"Adds Demi Summons to the Main Combo.","在循环中添加龙神召唤、不死鸟召唤。"},
        {"Demi Summons AoE Combo","亚灵神召唤进 AoE 循环"},
        {"Adds Demi Summons to the AoE Combo.","在 AoE 循环中添加龙神召唤、不死鸟召唤。"},
        {"Swiftcast Egi Ability Option","即刻咏唱蛮神之灵技能选项"},
        {"Swiftcasts during the selected Primal Summon.","在选择的蛮神召唤阶段使用即刻咏唱。"},
        {"Astral Flow/Enkindle on Bahamut/Phoenix","星极超流/龙神迸发/不死鸟迸发应用于龙神/不死鸟"},
        {"Adds Astral Flow and Enkindle to Bahamut/Phoenix.","在龙神/不死鸟阶段使用星极超流/龙神迸发/不死鸟迸发"},
        {"Pooled oGCDs Feature","积攒能力技功能"},
        {"Pools damage OGCDs to use under Searing Light and in Bahamut/Phoenix Phase.\nChoose which phase to burst in under 'Burst Phase Choice' option.","积攒能力技打进灼热之光和龙神/不死鸟阶段。。\n“爆发阶段选择”选项中可以具体设置。"},
        {"Precious Brilliance on AoE Combo","宝石辉进 AoE 循环"},
        {"Adds Egi attacks (Precious Brilliance) to the AoE Combo.","添加蛮神之灵群体攻击（宝石辉）进 AoE 循环。"},
        {"Changes Swiftcast to Raise when on cooldown","即刻复活"},
        {"Rekindle on Main Combo option","苏生之炎进循环选项"},
        {"Adds Rekindle to the Main Combo.","将苏生之炎加入单体循环。"},
        {"Rekindle on AoE Combo option","苏生之炎进 AoE 循环选项"},
        {"Adds Rekindle to the AoE Combo.","将苏生之炎加入 AoE 循环"},
        {"Ruin III Mobility Feature","毁荡走位设置"},
        {"Puts Ruin III on Ruin IV when you don't have Further Ruin.","当你没有毁绝预备buff时，用毁荡替换毁绝。"},
        {"Adds Lucid dreaming to the Main Combo when below set MP value.","在设定 MP 以下时，将醒梦加入循环。"},
        {"Burst Phase Choice","爆发阶段选择"},
        {"Chooses which phase to burst in for all relevant burst features. Fester and Searing Light will only be used during Bahamut/Phoenix windows.","为所有关于爆发的功能设置爆发阶段。溃烂爆发和灼热之光只会用在龙神/不死鸟的窗口期。"},
        {"Egi Abilities on Egi Summons","蛮神能力技替换蛮神召唤"},
        {"Adds Egi Abilities (Astral Flow) to Egi Summons when ready.\nEgi Abilities will appear on their respective Egi Summon Ability, as well as, Titan.","蛮神能力技（星极超流）在能用的时候添加到蛮神召唤上。\n各个蛮神能力技将出现在各自的蛮神召唤技能以及土神召唤上。"},
        {"Egi and Demi Summon features","蛮神之灵召唤、亚灵神召唤功能"},
        {"Features related to changing Egi and Demi summons.\nCollapsing this category does NOT disable the features inside.","关于改变蛮神之灵召唤、亚灵神召唤的功能。\n取消勾选本条目不会取消内部的设置。"},
        {"Single target only Searing Light Option","灼热之光仅限单目标"},
        {"Only use Searing Light on Single Target combo.","只在单目标循环中使用灼热之光。"},
        {"Single target only Pooled oGCD Option","积攒能力技仅限单目标"},
        {"Only use damage oGCDs on single target combo.","只在单目标循环中积攒能力技。"},
        {"Single target only Swiftcast Egis Option","即刻咏唱蛮神技能仅限单目标"},
        {"Only use Swiftcast on single target combo.","只在单目标循环中使用即刻咏唱"},
        {"Ruin 4 Painflare Option","毁绝 痛苦核爆 选项"},
        {"Changes Painflare to Ruin 4 when out of Aetherflow stacks, Energy Siphon is on cooldown, and Ruin 4 is up.","当以太超流档数用光，能量吸收在冷却，毁绝能用时，用毁绝替换痛苦核爆。"},
        #endregion
        #region WARRIOR
        ...
        {"Adds Delirium to AoE combo on cooldown and when Darkside is up.","若自身存在暗影状态且血乱未在冷却中,在刚魂连击中插入血乱."},
        ...
        {"Adds Bow Shock to the main combo","在主连击中插入弓形冲波."},
        {"Adds Sonic Break to the main combo","在主连击中插入音速破"},
        ...
        {"Delays all oGCDs until after third Glare III cast.","延后所有能力技直到打出第三个闪灼."},
        ...
        {"Changes Cure II into Afflatus Solace when Lilies are up.","当有治疗百合存在时，替换救疗为安慰之心."},
        ...
        {"Displays oGCD Heals/Shields over Afflatus","当同时勾选优先使用神名、神祝祷时，先显示神祝祷."},
