...
public class Translatezh_CN
{
    public static Dictionary<string, string> db = new()
    {
        ...
        {"Play turns into Draw when no card is drawn","当没有抽取到奥秘卡时，自动将出卡替换为抽卡。"},
        ...
        {"Removed DoT From the DPS Feature","将DOT移除DPS连击，其他的DPS不受影响"},
        ...
        {"Adds Lady of Crowns","当抽取到王冠之贵妇时，自动加入治疗连击"},
        {"Simple Heal","简单治疗"},
        ...
        {"Fire 1/3","火1/3"},
        {"Fire 1 becomes Fire 3 outside of Astral Fire","当星极火状态消失或火苗状态消失时，自动替换火1为火3。"},
        {"Thunder","雷DOT"},
        {"Thunder 1/3 replaces Enochian/Fire 4/Blizzard 4 on Enochian switcher.\n Occurs when Thundercloud is up and either\n- Thundercloud buff on you is about to run out","在天语转换中启用自动替换火4/冰4为雷1/3。\n 出现了以下两种情况时，才会激活\n- 你身上的雷云BUFF马上要超时，或\n- 你目标身上的雷属性DEFUFF马上要消失\n假设它不会中断BUFF计时器。\n天语转换必须处于激活状态"},
        {"Despair Feature","绝望"},
        ...
        {"Adds every single target ability to one button","一键整合所有单体攻击技能。\n如果目标身上有dots，会同时尝试保持dots不断。"},
        ...
        {"Removes enemy health checking on mobs for buffs","去除目标HP检测，在战斗即将结束时不自动替换dot/buff/歌技能。"},
        ...
        {"Enable the snapshotting of DoTs","在猛者强击持续时间低于设定值时，自动替换插入dot技能。"},
        //#endregion
        //#region DANCER
        ...
        {"BETA - Single button","测试版 - 单体目标，一键连击。 包含跳舞，进攻之探戈，触发/溢出保护。\n与除了'跳舞连击'外的所有单体连击冲突\n后果自负!"},
        ...
        {"Makes the rotation only use feathers when you have more than 3","只在幻扇层数大于3或技巧舞步状态下使用消耗幻扇层数的技能。"},
        {"Simple Panic Heals","简易紧急恢复"},
        {"Includes Curing Waltz and Second Wind in the rotation when available and below 30 and 50 percent HP","当血量低于30%或50%时对应的使用治疗之华尔兹或内丹。"},
        {"Simple Improvisation","简易即兴表演"},
        {"Includes Improvisation in the rotation when available.","当即兴表演可用时将其加入循环。"},
        {"Simple Dancer (AoE)","AoE一键连击"},
        {"BETA - Single button aoe dancer","测试版 - AoE，一键连击。 包含跳舞，进攻之探戈，触发/溢出保护。\n与除了'跳舞连击'外的所有AoE连击冲突。\n后果自负!"},
        ...
        {"Delays Bloodspiller by 2 GCDs when Delirium is used during even windows","在偶数分钟窗口使用血乱时将延迟两个GCD，而在奇数分钟窗口不进行延后。这将可以使技能更容易打进团辅。"},
        ...
        {"If TBN is used outside of combat","如果在战斗外使用了至黑之夜，那么将嗜血整合到主连击作为起手。"},
        ...
        {"Adds Salted Earth on Main Combo while Darkside is up","当暗黑激活时将腐秽大地整合到主连击。将会在你解锁了腐秽黑暗之后生效。"},
        ...
        {"Includes Life Surge","在樱花/直刺/山境酷刑连击中的合适时机和buff下加入龙剑。"},
        ...
        {"One Button","同一个按键，同一种龙骑。 与任何单体输出选项都有冲突。"},
        ...
        {"Single Weave Friendly","单插技能貌似还行: 在连击中插入破碎冲、龙炎冲、坠星冲。"},
        {"Dives under Lance Charge Feature","舍身状态下跳跃类能力技设置"},
        {"Single Weave Friendly: Includes Spineshatter Dive and Dragonfire Dive in the rotation","单插还不错: 在舍身状态下的连击中插入破碎冲/龙炎冲，在红莲龙血状态下插入坠星冲。"},
        {"Dives under Litany Feature","战斗连祷状态下跳跃类能力技设置"},
        {"Double Weaves Required: Includes Spineshatter Dive and Dragonfire Dive in the rotation","需要双插: 在战斗连祷状态下的连击中插入破碎冲/龙炎冲"},
        ...
        {"Includes Dragon Sight to the rotation. OPTIONAL: USE REACTION OR MOACTION FOR OPTIMAL TARGETING.","将巨龙视线加入循环。可选项：使用mo宏和手动进行最优的目标选择"},
        {"Life Surge Feature","龙剑设置"},
        ...
        {"Includes Spineshatter Dive","在连击中插入破碎冲/龙炎冲/坠星冲。"},
        {"Dives under Lance Charge AoE Feature","舍身状态下 跳跃类能力技设置"},
        {"Single Weave Friendly: Includes Spineshatter Dive and Dragonfire Dive in the AoE rotation","单插还不错:在舍身状态下连击中插入破碎冲/龙炎冲，同时在红莲龙血状态下插入坠星冲。"},
        {"Dives under Litany AoE Features","战斗连祷中 跳跃类能力技设置"},
        {"Includes Spineshatter Dive and Dragonfire Dive in the AoE rotation","在战斗连祷状态下的连击中插入破碎冲/龙炎冲"},
        ...
        {"When you have either Enhanced Fang and Claw or Wheeling Thrust","在龙牙龙爪或龙尾大回旋预备状态下，替换樱花连为龙尾大回旋，替换直刺连为龙牙龙爪. 需要开启一键樱花连和直刺连。"},
        ...
        {"Adds Sonic Break/Bow Shock/Blasting Zone on Gnashing Fang","当拥有无情BUFF时将音速破/弓形冲波/爆破领域整合到烈牙。 \n当拥有无情BUFF时爆发击也将会被整合到烈牙。"},
        ...
        {"Air Anchor followed by Drill is added onto main combo if you use Reassemble.\nIf Air Anchor is on cooldown and you use Reassemble","如果使用整备，空气锚后的钻头将会被加入到连击循环.\nIf Air Anchor is on cooldown and you use 整备"},
        ...
        {"Overcharge protection for your Battery","为您提供电能过量时的电能保护，如果您的电量达到100，则会在您的过热循环中加入炮台/女王。"},
        ...
        {"Single button single target machinist","一个按键，单个目标，自己上buff并避免过热，与单目标切换关联，使用2.5gcd为最佳配合"},
        {"Uses interrupt during simple machinist rotation","如果适用的话，在机工的简单循环中使用打断技能."},
        {"Simple Gadget","简单的机工召唤"},
        {"Adds Queen or Rook uses to the feature","根据你目前的等级，增加超档车式炮塔和超档自走人偶的技能.\n试着在:55到:05窗口之间的最佳间隔期使用超档车式炮塔和超档自走人偶."},
        ...
        {"Adds Hypercharge and Wildfire uses to the feature.\nIt respects the 8 second rule of Drill","将超荷和野火加入到循环中.\n它遵从钻头、空气锚和链锯的8秒规则."},
        ...
        {"Spread Shot turns into Scattergun when lvl 82 or higher","当等级为82或者更高时将散射转换为霰弹枪"},
        {"Drill","钻头"},
        ...
        {"Adds Soul Slice to Slice Combo when Soul Gauge is 50 or less","当魂衣值低于50点，并且目标拥有死亡烙印Debuff时，使用灵魂切割替换切割。"},
        ...
        {"Adds Bloodbath and Second Wind to the combo at 65 and 40 percent HP","在血量低于65%和40%时，使用浴血和内丹替换切割。"},
        {"Ranged Filler Option","距离设置"},
        {"Replaces the combo chain with Harpe (or Harvest Moon","当超出近战范围时，使用勾刃（当可用时，使用收获月）替换切割。不会替换掉团契。"},
        ...
        {"Adds Soul Scythe to Scythe Combo when Soul Gauge is 50 or less","当魂衣值低于50点，并且目标拥有死亡烙印Debuff时，使用灵魂钐割替换旋转钐割。"},
        ...
        {"Features and options involving Gibbet","包括绞决"},
        ...
        {"Slice is instead replaced with whichever move is procced","使用正确的绞决/缢杀替换切割，死亡之影不会被替换。"},
        ...
        {"Features and options involving Blood Stalk","包括束缚挥割，隐匿挥割与暴食的设置。\n禁用此项 不会 停用次级选项功能。"},
        {"Blood Stalk/Grim Swathe Feature","隐匿挥割/束缚挥割特性"},
        {"When Gluttony is off-cooldown","当暴食不在冷却时，使用暴食替换隐匿挥割/束缚挥割。"},
        ...
        {"Lemure Feature","收割特性"},
        {"When you have two or more stacks of Void Shroud","当你拥有两层虚无魂状态时，使用夜游魂切割替换缢杀/绞决，同时使用夜游魂钐割替换断首。"},
        {"Combo Communio Feature","团契特性"},
        {"When one stack of Lemure Shroud remains","当只剩下一层夜游魂时，使用团契替换缢杀/绞决与断首。"},
        {"Enshroud Communio Feature","夜游魂衣团契特性"},
        {"Replace Enshroud with Communio when Enshrouded.","当拥有夜游魂状态时，使用团契替换夜游魂衣。"},
        {"Turns Enshroud into Void/Cross Reaping","Turns 夜游魂衣 into Void/Cross Reaping"},
        {"[Extra Features]","[额外特性]"},
        ...
        {"Both Hell's Ingress and Hell's Egress turn into Regress when Threshold is active","当你拥有回退预备状态时，使用回退同时替换地狱入境与地狱出境，而不是和你使用的位移技能相反的那个。"},
        {"Harpe Soulsow Feature","勾刃播魂种特性"},
        {"Changes Harpe into Soulsow when you are out of combat or have no target","在战斗外使用播魂种替换勾刃。当你拥有播魂种BUFF时，不进行替换。"},
        ...
        {"Replaces Veraero/Verthunder 2 with Impact when Dualcast or Swiftcast are active.","若有连续咏唱或即刻咏唱状态"},
        {"Redoublement combo","魔连攻连击"},
        {"Replaces Redoublement with its combo chain","当黑白魔元满足条件时，替换连攻为魔连攻并可一键释放近战三连击。"},
        {"Redoublement Combo Plus","魔连攻连击特性"},
        {"Replaces Redoublement with Verflare/Verholy after Enchanted Redoublement","根据黑白魔元平衡自动将赤神圣（核爆）与焦热接在近战三连击之后。 \n需先启用魔连攻连击。"},
        ...
        {"Combines Smartcast feature with melee combo on Verstone","一键智能整合所有单体输出技能到摇荡（回刺、交击斩、连攻、赤火炎/疾风、震荡均可），并在黑白魔元满足条件且在近战范围时自动变为魔六连爆发。(高延迟下待优化)"},
        ...
        {"Replaces Jolt with Verstone/Verfire","当有触发时自动替换摇荡为赤飞石（火炎），若会引起魔元失衡则不会替换。"},
        {"Add Lucid Dreaming to Veraero","当MP低于设定数值时自动替换赤疾风、赤闪雷和冲击为醒梦。"},
        ...
        {"Replaces Taurochole","当蛇胆为0时，替换白牛清汁/灵橡清汁/寄生清汁/坚角清汁为根素。"},
        ...
        {"Does nothing on it's own","单独启用此选项无任何效果，请根据需要开启相应子选项。"},
        ...
        {"Change Fester/PainFlare into Ruin IV when out of Aetherflow stacks","当以太超流的档数为零，且能量吸收/能量抽取在CD，且毁绝触发时，将溃烂爆发/痛苦核爆自动替换为毁绝"},
        ...
        {"Adds Searing Light to Simple Summoner Rotation","将灼热之光加入到一键单体循环中"},
        ...
        {"Changes Cure 2 into Afflatus Solace","当拥有治疗百合时，将救疗替换为安慰之心，医治替换为狂喜之心"},
